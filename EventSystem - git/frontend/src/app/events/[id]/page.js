...
import {
  ...
  TicketIcon, ShareIcon, HeartIcon, AcademicCapIcon
...
export default function EventDetail({ params }) {
  ...
  const handlePurchase = async () => {
    ...
    try {
      // ✅ بعد — event_id رفت توی URL
        await axios.post(`${API_URL}/api/purchase/${event.id}/`,
        {},
  ...
  if (lecturerImage && !lecturerImage.startsWith('http')) lecturerImage = `${API_URL}${lecturerImage}`;
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent flex items-end">
          <div className="container mx-auto px-4 pb-12 text-white">
            <div className="flex items-center gap-2 mb-4">
              <span className="bg-blue-600 px-4 py-1.5 rounded-full text-xs font-bold">{categoryTitle}</span>
              <span className="bg-white/20 backdrop-blur-md px-4 py-1.5 rounded-full text-xs font-bold flex items-center gap-2">
                <UserIcon className="w-3 h-3" />{lecturer.name || 'مدرس نامشخص'}
              </span>
            </div>
            ...
            <div className="flex flex-wrap gap-6 text-sm md:text-base font-medium opacity-90">
              <span className="flex items-center gap-2 bg-black/30 px-3 py-1 rounded-lg backdrop-blur-sm">
                <CalendarIcon className="w-5 h-5 text-blue-400" />
                {new Date(event.start_time).toLocaleDateString('fa-IR', { dateStyle: 'long' })}
              </span>
              <span className="flex items-center gap-2 bg-black/30 px-3 py-1 rounded-lg backdrop-blur-sm">
                <MapPinIcon className="w-5 h-5 text-red-400" />{event.location}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="container mx-auto px-4 -mt-10 relative z-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
            ...
            <p className="text-gray-600 leading-8 text-justify whitespace-pre-line text-lg">{event.description}</p>
          </div>
            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
              ...
              <div className="flex flex-col md:flex-row gap-6 items-start">
                <div className="w-24 h-24 md:w-32 md:h-32 flex-shrink-0 relative">
                  <img src={displayLecturerImage} className="w-full h-full object-cover rounded-full border-4 border-blue-50 shadow-md" alt={lecturer.name} />
                ...
                <div className="flex-1">
                  ...
                  <p className="text-blue-600 text-sm font-bold mb-4 bg-blue-50 inline-block px-3 py-1 rounded-lg">{lecturer.specialty}</p>
                  <p className="text-gray-500 text-sm leading-7 text-justify">{lecturer.bio || "بیوگرافی برای این مدرس ثبت نشده است."}</p>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="lg:col-span-1">
          <div className="bg-white p-6 rounded-3xl shadow-lg shadow-gray-200/50 border border-gray-100 sticky top-24">
            ...
            <p className="text-center text-xs text-gray-400 mt-4 leading-5">با خرید بلیت، قوانین و مقررات رویداد را می‌پذیرید.</p>
          </div>
        </div>
      </div>
    </div>
