...
class AIService:
    ...
    def _send_request(self, system_msg, user_msg):
        """تابع کمکی برای ارسال درخواست به هوش مصنوعی با قابلیت هندل کردن خطاهای جدید"""
        try:
            ...
            # 1. اگر خروجی مستقیماً متن بود (تغییر جدید GapGPT)
            if isinstance(response, str):
                return response.strip()

            # 2. اگر خروجی آبجکت استاندارد OpenAI بود (حالت معمول)
            if hasattr(response, 'choices'):
                return response.choices[0].message.content.strip()
            
            # 3. اگر خروجی دیکشنری بود (حالت‌های خاص)
            if isinstance(response, dict):
                return response.get('choices', [{}])[0].get('message', {}).get('content', '').strip()

            print(f"⚠️ Unknown API Response: {response}")
            return None

        except Exception as e:
            print(f"⚠️ AI Connection Error: {e}")
            # برای دیباگ دقیق‌تر، نوع ارور را چاپ می‌کنیم
            import traceback
            traceback.print_exc()
            return None
